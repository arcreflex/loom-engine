...
Individual communication units within a Node.
...
A request from an AI model to execute a specific tool with given parameters. Represented as a `tool-use` content block within an assistant message. Each has a unique `id` for correlation with tool results.
